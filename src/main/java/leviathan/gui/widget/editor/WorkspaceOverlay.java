...
public class WorkspaceOverlay extends WidgetContainer {
	...
	public WorkspaceOverlay(WorkspaceCanvas canvas) {
		super(0, 0, 32, 32);
		this.canvas = canvas;
		for (WidgetAlignment alignment : WidgetAlignment.values()) {
			...
			widget.addListener(MouseEvent.MOUSE_DRAG_MOVE, event -> {
				...
				Optional<IWidget> optionalSelected = canvas.getSelectedWidget();
				if (optionalSelected.isPresent()) {
					...
					canvas.setSelectedRegion(selectedWidget, new Region(x, y, w, h));
				}
			});
			add(widget);
		}
		...
		addListener(MouseEvent.MOUSE_DRAG_MOVE, event -> {
			Optional<IWidget> optionalSelected = canvas.getSelectedWidget();
			if (!isVisible() || !optionalSelected.isPresent()) {
				return;
			}
			IWidget selectedWidget = optionalSelected.get();
			canvas.setSelectedRegion(selectedWidget, selectedWidget.getRegion().withPosition(Math.round(selectedWidget.getX() + event.getDiffX()), Math.round(selectedWidget.getY() + event.getDiffY())));
		});
	}
